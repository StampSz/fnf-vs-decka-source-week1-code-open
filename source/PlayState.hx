			...
			case 'decka1':
				{
						defaultCamZoom = 0.9;
						curStage = 'decka1';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('amogus', 'shared'));
						bg.antialiasing = true;
			...
			case 'decka2':
				{
						defaultCamZoom = 0.9;
						curStage = 'decka2';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bank', 'shared'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (FlxG.save.data.scoreScreen && SONG.song.toLowerCase() != 'busted')
